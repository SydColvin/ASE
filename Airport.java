...
public class Airport {
	...
	private Semaphore lookforEmptyDesk = new Semaphore(1);
	...
	class CheckIn {
		...
		public String currantDesk = null;

		// To count the total number of check-ins for testing
		int noCheckIns = 0;

		public int getC() {
			return this.noCheckIns;
		};
		...
		public void useDesk() {
			...
			String message = "Check-In " + noCheckIns + " (" + threadName + ") at desk " + currantDesk;
		...
		public String identifyFreeDeskNo() {
			for (int i = 0; i < deskStatus.length; i++) {
				// System.out.println("Desk stats " + deskStatus[i]);
				if (deskStatus[i] == null) {
					this.currantDesk = Integer.toString(i+1);
					break;
				}
			}
			 System.out.println(Thread.currentThread().getName() + 
					 "going to desk No. " + currantDesk);
			return currantDesk;
		}
		...
		public void emptyIdentifiedDesknoNo(int n) {
			deskStatus[n - 1] = null;
			this.currantDesk = null; // System.out.println("emtied desk no.:-" + n + " [" + currantDesk + "]");
		}
	}
	...
	class PassengerInQueue implements Runnable {
		public void run() {
			...
			try {
				...
				lookforEmptyDesk.acquire();
				int deskno = Integer.parseInt(checkin.identifyFreeDeskNo());
				lookforEmptyDesk.release();
	...
	public String getQueueStatus() {
		String status = "";
		int n = passengerQueue.getPassengerListSize();
		status += "There are currently " + n + " people waiting in the queue: ";
		int i = 0;
		HashSet<Passenger> pQ = PassengerList.getPassengerList();
		for (Passenger p : pQ) {
			if (i == 0) {
